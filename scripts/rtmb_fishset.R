...
df <- data.frame(
  zones = rep(unique_zones, length(main_data$TRIPID)),
...
distance_long <- as.data.frame(mdf$distance) %>%
  mutate(TRIPID = main_data$TRIPID) %>%
  pivot_longer(
    cols = -c(TRIPID),
    names_to = "zones",
...
df_long <- df %>%
  ...
  left_join(distance_long, by = c("zones" = "zones", "obsID" = "TRIPID")) %>%
  left_join(exp_catch_long, by = c("zones" = "zones", "obsID" = "TRIPID")) %>%
  rename(ZoneID = zones, TRIPID = obsID) %>%
  select(ZoneID, TRIPID, selected, distance_from_port, expected_catch) %>%
