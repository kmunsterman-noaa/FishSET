...
# Scen 1
# 1. Wrap the script in a loop to run 20 times
simulation_results <- lapply(1:20, function(run_number) {
  
  # --- Your Original Code Start ---
  welfare_output_scen1 <- calculate_welfare_change(
    results_profit, 
    covariates, 
    closed_zones_scen1, 
    cost_variable_index = 1,
  ...
  summary_stats <- main_data %>%
    ...
    mutate(welfare_loss_per_trip = -1 * welfare_per_trip) %>%
    summarise(
      mean_welfare_per_trip = mean(welfare_loss_per_trip), 
      sd = sd(welfare_loss_per_trip)
    )
  
  # Add the run index to the output
  summary_stats %>% mutate(simulation_run = run_number)
})

# 2. Combine the list into a single data frame
plot_data <- bind_rows(simulation_results)

# 3. Plot the results
ggplot(plot_data, aes(x = simulation_run, y = mean_welfare_per_trip)) +
  geom_point(color = "steelblue", size = 3) +
  geom_line(alpha = 0.3) + # Optional: connects dots to show variance flow
  theme_classic() +
  labs(
    title = "Scenario 1",
    x = "Simulation Run",
    y = "Mean Profit Loss per Trip"
  ) +
...
simulation_results <- lapply(1:20, function(run_number) {
  ...
  # --- Your Original Code Start ---
  welfare_output_scen2 <- calculate_welfare_change(
    results_profit, 
    covariates, 
    closed_zones_scen1, 
    cost_variable_index = 1,
...
###### Testing closing zones with highest fishing

# TESTING WELFARE VALUES -------------------------------------------------------

#### SCENARIO 1 FISHING EFFORT (TRIPS)

# by fishing effort 
zones <- zOut$table

# determine the number of zones to close based on the number in scenario 1
X <- length(closed_zones_scen1)

scenario_1_effort <- zones %>%
  # Sort by 'n' in descending order (highest values first)
  arrange(desc(n)) %>%
  # Keep only the top X rows
  slice_head(n = X) %>%  
  rename(GRID5KM_ID = new_zoneID) %>%
  dplyr::select(-n) %>%
  mutate(scenario = "closure_1_effort")

scenario_name <- "closure_1_effort"

filename <- paste0(locoutput(project), scenario_name, "_closures.yaml")

yaml::write_yaml(scenario_1_effort, filename)
...
unique_zones <- unique(main_data$new_zoneID)
closed_zones_scenario_1_effort <- intersect(closed_zones, unique_zones)

# --- Predict redistributed probabilities under the closure ---
# First item in list is redistributed probabilities for each trip
# Second item is redistributed probabilities for each zone
redistributed_probabilities <- predict_redistributed_probs(results, covariates, closed_zones_scenario_1_effort)

# 1. Wrap the script in a loop to run 20 times
simulation_results <- lapply(1:20, function(run_number) {
  
## Profit
welfare_output_scenario_1_effort <- calculate_welfare_change(results_profit, 
                                                 covariates, 
                                                 closed_zones_scenario_1_effort, 
                                                 cost_variable_index = 1,
                                                 is_cost_variable = FALSE,
                                                 beta_samples = 1000)

# Calculate mean welfare loss per TRIP
welfare_per_haul <- welfare_output_scenario_1_effort[[2]]
mean_welfare_per_haul <- rowMeans(welfare_per_haul, na.rm = TRUE)
...
ggplot(plot_data, aes(x = simulation_run, y = mean_welfare_per_trip)) +
  ...
  theme_classic() +
  labs(
    title = "Scenario 1_Effort",
    x = "Simulation Run",
    y = "Mean Profit Loss per Trip"
  ) +
  scale_x_continuous(breaks = 1:20)

#### SCENARIO 2 FISHING EFFORT (TRIPS)

# by fishing effort 
zones <- zOut$table

# determine the number of zones to close based on the number in scenario 2
X <- length(closed_zones_scen2)

scenario_2_effort <- zones %>%
  # Sort by 'n' in descending order (highest values first)
  arrange(desc(n)) %>%
  # Keep only the top X rows
  slice_head(n = X) %>% 
  rename(GRID5KM_ID = new_zoneID) %>%
  dplyr::select(-n) %>%
  mutate(scenario = "closure_2_effort")

scenario_name <- "closure_2_effort"

filename <- paste0(locoutput(project), scenario_name, "_closures.yaml")

yaml::write_yaml(scenario_2_effort, filename)
...
unique_zones <- unique(main_data$new_zoneID)
closed_zones_scenario_2_effort <- intersect(closed_zones, unique_zones)

# --- Predict redistributed probabilities under the closure ---
# First item in list is redistributed probabilities for each trip
# Second item is redistributed probabilities for each zone
redistributed_probabilities <- predict_redistributed_probs(results, covariates, closed_zones_scenario_2_effort)

# 1. Wrap the script in a loop to run 20 times
simulation_results <- lapply(1:20, function(run_number) {
  
  ## Profit
  welfare_output_scenario_2_effort <- calculate_welfare_change(results_profit, 
                                                               covariates, 
                                                               closed_zones_scenario_2_effort, 
                                                               cost_variable_index = 1,
                                                               is_cost_variable = FALSE,
                                                               beta_samples = 1000)
  
  # Calculate mean welfare loss per TRIP
  welfare_per_haul <- welfare_output_scenario_2_effort[[2]]
  mean_welfare_per_haul <- rowMeans(welfare_per_haul, na.rm = TRUE)
...
ggplot(plot_data, aes(x = simulation_run, y = mean_welfare_per_trip)) +
  ...
  theme_classic() +
  labs(
    title = "Scenario 2_Effort",
    x = "Simulation Run",
    y = "Mean Profit Loss per Trip"
  ) +
  scale_x_continuous(breaks = 1:20)

#### SCENARIO 1 EXPECTED REVENUE

# by expected revenue 
zones <- predicted_probs_zones

# determine the number of zones to close based on the number in scenario 1
X <- length(closed_zones_scen1)

scenario_1_rev <- zones %>%
  # Sort by 'n' in descending order (highest values first)
  arrange(desc(Probability)) %>%
  # Keep only the top X rows
  slice_head(n = X) %>%  
  rename(GRID5KM_ID = ZoneID) %>%
  dplyr::select(-Probability) %>%
  mutate(scenario = "closure_1_rev")

scenario_name <- "closure_1_rev"

filename <- paste0(locoutput(project), scenario_name, "_closures.yaml")

yaml::write_yaml(scenario_1_rev, filename)
...
unique_zones <- unique(main_data$new_zoneID)
closed_zones_scenario_1_rev <- intersect(closed_zones, unique_zones)

# --- Predict redistributed probabilities under the closure ---
# First item in list is redistributed probabilities for each trip
# Second item is redistributed probabilities for each zone
redistributed_probabilities <- predict_redistributed_probs(results, covariates, closed_zones_scenario_1_rev)

# 1. Wrap the script in a loop to run 20 times
simulation_results <- lapply(1:20, function(run_number) {
  
  ## Profit
  welfare_output_scenario_1_rev <- calculate_welfare_change(results_profit, 
                                                               covariates, 
                                                               closed_zones_scenario_1_rev, 
                                                               cost_variable_index = 1,
                                                               is_cost_variable = FALSE,
                                                               beta_samples = 1000)
  
  # Calculate mean welfare loss per TRIP
  welfare_per_haul <- welfare_output_scenario_1_rev[[2]]
  mean_welfare_per_haul <- rowMeans(welfare_per_haul, na.rm = TRUE)
...
ggplot(plot_data, aes(x = simulation_run, y = mean_welfare_per_trip)) +
  ...
  theme_classic() +
  labs(
    title = "Scenario 1_REVENUE",
    x = "Simulation Run",
    y = "Mean Profit Loss per Trip"
  ) +
  scale_x_continuous(breaks = 1:20)

#### SCENARIO 2 FISHING rev (TRIPS)
...
scenario_2_rev <- zones %>%
  arrange(desc(Probability)) %>%
  # Keep only the top X rows
  slice_head(n = X) %>% 
  rename(GRID5KM_ID = ZoneID) %>%
  dplyr::select(-Probability) %>%
  mutate(scenario = "closure_2_rev")

scenario_name <- "closure_2_rev"

filename <- paste0(locoutput(project), scenario_name, "_closures.yaml")

yaml::write_yaml(scenario_2_rev, filename)
...
unique_zones <- unique(main_data$new_zoneID)
closed_zones_scenario_2_rev <- intersect(closed_zones, unique_zones)

# --- Predict redistributed probabilities under the closure ---
# First item in list is redistributed probabilities for each trip
# Second item is redistributed probabilities for each zone
redistributed_probabilities <- predict_redistributed_probs(results, covariates, closed_zones_scenario_2_rev)

# 1. Wrap the script in a loop to run 20 times
simulation_results <- lapply(1:20, function(run_number) {
  
  ## Profit
  welfare_output_scenario_2_rev <- calculate_welfare_change(results_profit, 
                                                               covariates, 
                                                               closed_zones_scenario_2_rev, 
                                                               cost_variable_index = 1,
                                                               is_cost_variable = FALSE,
                                                               beta_samples = 1000)
  
  # Calculate mean welfare loss per TRIP
  welfare_per_haul <- welfare_output_scenario_2_rev[[2]]
  mean_welfare_per_haul <- rowMeans(welfare_per_haul, na.rm = TRUE)
...
### Extreme Scenario -- Close all but ONE zone

# We pull the ID from the 'zones' dataframe
zones <- zOut$table
open_zone_id <- sample(zones$new_zoneID, 1)

# 2. Create the extreme scenario: All zones EXCEPT the open one
scenario_EX <- zones %>%
  # Filter out the one zone we want to keep open
  filter(new_zoneID != open_zone_id) %>%
  # Rename and select as per your previous structure
  rename(GRID5KM_ID = new_zoneID) %>%
  dplyr::select(-n) %>%
  mutate(scenario = "closure_EX")

scenario_name <- "closure_EX"

filename <- paste0(locoutput(project), scenario_name, "_closures.yaml")

yaml::write_yaml(scenario_EX, filename)
...
unique_zones <- unique(main_data$new_zoneID)
closed_zones_scenario_EX <- intersect(closed_zones, unique_zones)

# --- Predict redistributed probabilities under the closure ---
# First item in list is redistributed probabilities for each trip
# Second item is redistributed probabilities for each zone
redistributed_probabilities <- predict_redistributed_probs(results, covariates, closed_zones_scenario_EX)
 
  ## Profit
  welfare_output_scenEX <- calculate_welfare_change(results_profit, 
                                                               covariates, 
                                                               closed_zones_scenario_EX, 
                                                               cost_variable_index = 1,
                                                               is_cost_variable = FALSE,
                                                               beta_samples = 1000)
  
  # Calculate mean welfare loss per TRIP
  welfare_per_haul <- welfare_output_scenEX[[2]]
  mean_welfare_per_haul <- rowMeans(welfare_per_haul, na.rm = TRUE)
  
  welfare_per_trip_scenEX <- main_data %>%
    dplyr::select(trip_id, haul_id) %>%
    mutate(mean_welfare_per_haul = mean_welfare_per_haul) %>%
    group_by(trip_id) %>%
    summarise(welfare_per_trip=sum(mean_welfare_per_haul)) %>%
